...
namespace Negocio
{
    public class ModeloDetalle_Factura : IModelo
	{
		private readonly RepositorioDetalle_Factura repositorio;
		public EDetalle_Factura Entidad { get; set; }
        ...
        {
			repositorio = new RepositorioDetalle_Factura();
			Entidad = new EDetalle_Factura();
		}
		...
		public EDetalle_Factura LeerEntidad()
		{
			try
			{
				Exito = true;
		...
		public List<EDetalle_Factura> LeerLista()
		{
			try
			{
				Exito = true;
			...
			catch (SqlException ex)
			{
				Mensaje = ex.Message;
				Exito = false;
				return new List<EDetalle_Factura>();
			}
		}
	}
}